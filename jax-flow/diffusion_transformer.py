...
# Modified for FSDiT: support-set conditioning via FiLM (replaces class labels).

#################################################################################
#               Embedding Layers for Timesteps and Support Sets                 #
#################################################################################
...
# ---- KEPT FOR BACKWARD COMPATIBILITY (used by original train_flow.py) ----
...
# ---- NEW: Support-set Projector for FSDiT ----
class SupportProjector(nn.Module):
    """
    Projects mean-pooled SigLIP2 support embeddings → DiT hidden_size.
    Handles CFG dropout (zero out conditioning 10% of time during training).
    """
    hidden_size: int
    siglip_dim: int = 768    # SigLIP2 B/16
    ...
    def __call__(self, support_embed, train=False, force_drop_ids=None):
        """
        Args:
            support_embed: (B, siglip_dim) — mean-pooled SigLIP2 embedding
            train: bool
            force_drop_ids: if True → force zero (uncond), if False → force keep
        Returns:
            (B, hidden_size)
        """
        ...
        if force_drop_ids is not None:
            if isinstance(force_drop_ids, bool):
                ...
                support_embed = jnp.where(drop_mask[:, None], 0.0, support_embed)
        elif train and self.dropout_prob > 0:
            rng = self.make_rng('cond_dropout')
            drop_ids = jax.random.bernoulli(rng, self.dropout_prob, (support_embed.shape[0],))
            support_embed = jnp.where(drop_ids[:, None], 0.0, support_embed)
        
        # MLP: siglip_dim → hidden*2 → GELU → hidden
...
class DiTBlock(nn.Module):
    ...
    Modified to optionally return attention weights for entropy tracking.
    """
    hidden_size: int
    num_heads: int
    mlp_ratio: float = 4.0
...
class DiT(nn.Module):
    ...
    num_classes: int = 0
    # FSDiT support-set conditioning
    siglip_dim: int = 0       # 0 = use class labels (original), >0 = use support-set (FSDiT)
    ...
    def __call__(self, x, t, y, train=False, force_drop_ids=None, return_attn=False):
        ...
        Args:
            ...
            y: (B,) int class labels  OR  (B, siglip_dim) float support embedding
            train: bool
        ...
        # Conditioning: class labels or support embeddings
        if self.siglip_dim > 0:
            # FSDiT mode: y is (B, siglip_dim) support embedding
            y = SupportProjector(
                self.hidden_size, 
                siglip_dim=self.siglip_dim,
                dropout_prob=self.cond_dropout_prob,
            )(y, train=train, force_drop_ids=force_drop_ids)
        else:
            # Original mode: y is (B,) class labels
        ...
        for _ in range(self.depth):
            if return_attn:
                x, attn_w = DiTBlock(self.hidden_size, self.num_heads, self.mlp_ratio)(x, c, return_attn=True)
